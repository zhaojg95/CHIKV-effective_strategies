...
psf_plot <- function(data,Name){
  ...
  ymax <- ceiling(max(data$high, na.rm = TRUE) / 1000 / 100) * 100
  brks <- seq(0, ymax, length.out = 4)
  ...
  p <- ggplot(data, aes(x = start_date + day-1)) +
    ...
    theme_bw()+
    theme(
      axis.text.x      = element_text(size = 6, colour = "black", angle = 0, hjust = 0.5, vjust = 0.5),
    ...
    scale_x_date(date_labels = "%b %d\n2025", date_breaks = "1 month", 
                 limits = c(as.Date("2025-06-01"), NA),
                 expand = c(0.1, 0.1)) +
    scale_y_continuous(
      limits = c(0, ymax),
      breaks = brks,
...
Name = "a"
a_sensitivity <- psf(Name,From_value = 0.0001,To_value = 2.86,length.out)
a <- psf_plot(a_sensitivity,Name = expression(a));a


# "mu_a" -- Mortality rate of aquatic stage ----------------------------
Name = "mu_a"
mu_a_sensitivity <- psf(Name,From_value = 0.013,To_value = 0.059,length.out)
...
ggsave(filename = "3.result/plot fig/sens_combined.pdf",sens_combined,width = 25,height = 20,dpi = 400,units = "cm")
